...
| `changelog/`, `_layouts/changelog.html` | The paginated changelog: merged PRs in public polarizetech repos, each entry in full. **Every `index.html` under `changelog/` carrying the GENERATED marker is a stub** — never edit or add one by hand (`changelog/sim-neural-memory/` is a hand-written redirect for a renamed repo). |
| `work/index.html`, `work/<repo>/`, `_layouts/repo.html`, `_includes/repo-posts.html` | **Work**: a bento board of every public repo at `/work/`, and one feature page per repo at `/work/<repo>/` (a generated stub). Each leads with the **posts about the repo** — found automatically: a post belongs to a repo when its text links to `github.com/polarizetech/<repo>` — then the README and the changelog. To attach a post to a repo, link the repo in the post. |
| `_data/changelog.json`, `_data/repos.json` | **Generated** by `scripts/sync_changelog.py` from GitHub (public repos only), together with the changelog and `work/<repo>/` stubs. `.github/workflows/changelog.yml` re-runs it once a day (and on demand). |
